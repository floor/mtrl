...
import { normalizeClasses } from "../../src/core/dom";
...
describe("normalizeClasses Utility", () => {
  ...
  test("should remove duplicate classes", () => {
    ...
    expect(normalizeClasses(["test", "test"])).toEqual(["test"]);
    expect(normalizeClasses("test1 test2", "test2")).toEqual([
      "test1",
      "test2",
    ]);
  });
  ...
  test("should handle mixed input types", () => {
    expect(normalizeClasses("test1", ["test2 test3", "test4"])).toEqual([
      "test1",
      "test2",
      "test3",
      "test4",
...
describe("classNames Utility", () => {
  ...
  test("should filter out falsy values", () => {
    expect(classNames("test1", null, "test2", undefined, false, "test3")).toBe(
      "test1 test2 test3"
    );
  });
