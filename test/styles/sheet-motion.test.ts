...
describe('dialog', () => {
  ...
  test('the dialog grows in height from the top and slides into place', () => {
    expect(value('dialog', '.mtrl-dialog', 'transform')).toBe('translateY(-50px) scaleY(0.35)');
    expect(value('dialog', '.mtrl-dialog', 'transform-origin')).toBe('top center');
    expect(value('dialog', '.mtrl-dialog--visible', 'transform')).toBe('translateY(0) scaleY(1)');
  });

  test('it grows on emphasized over 500ms and closes on emphasized accelerate over 150ms', () => {
    expect(value('dialog', '.mtrl-dialog--visible', 'transition')).toBe('transform 500ms cubic-bezier(0.3, 0, 0, 1), opacity 50ms linear');
    expect(value('dialog', '.mtrl-dialog', 'transition')).toBe('transform 150ms cubic-bezier(0.3, 0, 0.8, 0.15), opacity 50ms linear 100ms');
  });
