...
import type { SliderSize } from "../../src/components/slider/constants";
...
const createMockSlider = (config: SliderConfig = {}): SliderComponent => {
  ...
  const settings = {
    ...
    size: config.size || SLIDER_SIZES.M,
    ticks: config.ticks || false,
    valueFormatter:
      config.valueFormatter || ((value: number) => value.toString()),
  ...
  if (settings.ticks) {
    ...
    const numTicks =
      Math.floor((settings.max - settings.min) / settings.step) + 1;
  ...
  if (settings.showValue) {
    ...
    if (settings.range && secondThumb) {
      ...
      secondValueElement.textContent = settings.valueFormatter(
        settings.secondValue || settings.min
      );
  ...
  const updateSliderUI = () => {
    ...
    if (settings.range && settings.secondValue !== undefined) {
      ...
      const secondPercent =
        ((settings.secondValue - settings.min) / range) * 100;
    ...
    if (valueElement) {
      ...
      if (settings.range && secondThumb && settings.secondValue !== undefined) {
        ...
        if (secondValueEl) {
          secondValueEl.textContent = settings.valueFormatter(
            settings.secondValue
          );
        }
      }
    }
  ...
  const slider: SliderComponent = {
    ...
    setValue: (value: number, triggerEvent: boolean = false) => {
      ...
      if (settings.snapToSteps && settings.step > 0) {
        value =
          Math.round((value - settings.min) / settings.step) * settings.step +
          settings.min;
      }
    ...
    setMin: (min: number) => {
      ...
      if (
        settings.range &&
        settings.secondValue !== undefined &&
    ...
    setStep: (step: number) => {
      ...
      if (settings.snapToSteps) {
        settings.value =
          Math.round((settings.value - settings.min) / step) * step +
          settings.min;

        if (settings.range && settings.secondValue !== undefined) {
          settings.secondValue =
            Math.round((settings.secondValue - settings.min) / step) * step +
            settings.min;
        }
    ...
    showCurrentValue: (show: boolean) => {
      ...
      if (show) {
        ...
        if (settings.range && secondThumb) {
          const secondValueEl = secondThumb.querySelector(
            ".mtrl-slider__value"
          );
          if (secondValueEl) {
    ...
    setLabel: (text: string) => {
      ...
      if (text) {
        ...
        element.classList.remove(
          `mtrl-slider--label-${settings.labelPosition}`
        );
      }
...
describe("Slider Component", () => {
  ...
  test("should apply medium size by default", () => {
    const slider = createMockSlider();
    expect(slider.element.className).toContain("mtrl-slider--40");
  });
  ...
  test("should update second value for range slider", () => {
    ...
    const secondThumb = slider.element.querySelector(
      ".mtrl-slider__thumb--second"
    );
  ...
  test("should change size", () => {
    ...
    expect(slider.getSize()).toBe(SLIDER_SIZES.M);
    expect(slider.element.className).toContain("mtrl-slider--40");

    slider.setSize(SLIDER_SIZES.L);

    expect(slider.getSize()).toBe(SLIDER_SIZES.L);
    expect(slider.element.className).toContain("mtrl-slider--56");
    expect(slider.element.className).not.toContain("mtrl-slider--40");
  });
