...
const createMockSwitch = (config: SwitchConfig = {}): SwitchComponent => {
  ...
  if (mergedConfig.label) {
    ...
    if (mergedConfig.labelPosition) {
      element.classList.add(
        `${mergedConfig.prefix}-switch--label-${mergedConfig.labelPosition}`,
      );
    }
  }
  ...
  if (mergedConfig.supportingText) {
    ...
    if (mergedConfig.error) {
      supportingTextElement.classList.add(
        `${mergedConfig.prefix}-switch-helper--error`,
      );
  ...
  const switchComp: SwitchComponent = {
    ...
    setSupportingText: (text: string, isError = false) => {
      ...
      supportingTextElement.classList.toggle(
        `${mergedConfig.prefix}-switch-helper--error`,
        isError,
      );
...
describe("Switch Component", () => {
  ...
  test("should have value getter and setter", () => {
    ...
    // getValue returns boolean (unified API)
    expect(switchComp.getValue()).toBe(false);

    // setValue accepts boolean
    switchComp.setValue(true);
    expect(switchComp.getValue()).toBe(true);

    switchComp.setValue(false);
    expect(switchComp.getValue()).toBe(false);
  ...
  test("should have value attribute getter and setter", () => {
    const switchComp = createMockSwitch();

    // Check API methods for value attribute (rarely needed)
    expect(typeof switchComp.getValueAttribute).toBe("function");
    expect(typeof switchComp.setValueAttribute).toBe("function");

    // Set a value attribute
    const testValue = "dark-mode";
    switchComp.setValueAttribute(testValue);

    // Check value attribute
    expect(switchComp.getValueAttribute()).toBe(testValue);
  });
