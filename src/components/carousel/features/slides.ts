...
export const withSlides = (config: CarouselConfig) => (component) => {
  ...
  function updateSlideSizes() {
    ...
    let largeWidth =
      typeof CAROUSEL_DEFAULTS.ITEM_WIDTHS[layout].LARGE === "number"
        ? (CAROUSEL_DEFAULTS.ITEM_WIDTHS[layout].LARGE as number)
        : containerWidth;

    let mediumWidth =
      typeof CAROUSEL_DEFAULTS.ITEM_WIDTHS[layout].MEDIUM === "number"
        ? (CAROUSEL_DEFAULTS.ITEM_WIDTHS[layout].MEDIUM as number)
        : containerWidth * 0.75;

    let smallWidth =
      typeof CAROUSEL_DEFAULTS.ITEM_WIDTHS[layout].SMALL === "number"
    ...
    if (
      config.largeItemMaxWidth &&
      typeof config.largeItemMaxWidth === "number"
    ) {
    ...
    slides.forEach((slideEl, index) => {
      ...
      slideEl.className = `${component.getClass(
        "carousel"
      )}-slide ${component.getClass("carousel")}-slide--${size}`;

      // Handle text visibility based on size
      const titleEl = slideEl.querySelector(
        `.${component.getClass("carousel")}-slide-title`
      );
      const descEl = slideEl.querySelector(
        `.${component.getClass("carousel")}-slide-description`
      );
      ...
      if (descEl) {
        if (
          size === "small" ||
          (size === "medium" && layout === "multi-browse")
        ) {
  ...
  function addSlideElement(slide: CarouselSlide, index?: number): HTMLElement {
    ...
    slideElement.dataset.borderRadius = `${
      config.borderRadius || CAROUSEL_DEFAULTS.BORDER_RADIUS
    }`;
    ...
    contentContainer.className = `${component.getClass(
      "carousel"
    )}-slide-content`;
    ...
    if (slide.description) {
      ...
      description.className = `${component.getClass(
        "carousel"
      )}-slide-description`;
      ...
      slides.forEach((slideEl, i) => {
        if (i === currentSlide) {
          slideEl.classList.add(
            `${component.getClass("carousel")}-slide--active`
          );
    ...
    slides: {
      ...
      updateSlide: (index: number, slide: CarouselSlide) => {
        if (index >= 0 && index < slides.length) {
          ...
          // Update title
          const titleElement = slideElement.querySelector(
            `.${component.getClass("carousel")}-slide-title`
          );
          ...
          // Update description
          const descriptionElement = slideElement.querySelector(
            `.${component.getClass("carousel")}-slide-description`
          );
          ...
          // Update accent color
          const overlayElement = slideElement.querySelector(
            `.${component.getClass("carousel")}-slide-overlay`
          );
          if (overlayElement && slide.accent) {
            (overlayElement as HTMLElement).style.backgroundColor =
              slide.accent;
          }

          // Update button
          const buttonElement = slideElement.querySelector(
            `.${component.getClass("carousel")}-slide-button`
          );
          if (buttonElement) {
            if (slide.buttonText) {
              ...
              if (slide.accent) {
                (buttonElement as HTMLElement).style.backgroundColor =
                  slide.accent;
              }
            } else {
            ...
            const contentContainer = slideElement.querySelector(
              `.${component.getClass("carousel")}-slide-content`
            );
            if (contentContainer) {
              const button = document.createElement("a");
              button.className = `${component.getClass(
                "carousel"
              )}-slide-button`;
              button.textContent = slide.buttonText;
    ...
    updateLayout: (newLayout: CarouselLayout) => {
      ...
      Object.values(CAROUSEL_LAYOUTS).forEach((layoutValue) => {
        component.element.classList.remove(
          `${component.getClass("carousel")}-layout--${layoutValue}`
        );
      });

      // Add new layout class
      component.element.classList.add(
        `${component.getClass("carousel")}-layout--${newLayout}`
      );
  ...
  if (config.transition) {
    component.element.classList.add(
      `${component.getClass("carousel")}-transition--${config.transition}`
    );
  }
