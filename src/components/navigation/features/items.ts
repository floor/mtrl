  ...
  (component: BaseComponent): ItemsComponent => {
      ...
      if (itemConfig.items?.length) {
        itemConfig.items.forEach((nestedConfig) => {
          const container = item.closest(
            `.${prefix}-${NAV_CLASSES.ITEM_CONTAINER}`
          );
          if (container) {
            const nestedContainer = container.querySelector(
              `.${prefix}-${NAV_CLASSES.NESTED_CONTAINER}`
            );
            if (nestedContainer) {
              const nestedItem = nestedContainer.querySelector(
                `[data-id="${nestedConfig.id}"]`
              ) as HTMLElement;
              if (nestedItem) {
    ...
    if (component.emit) {
      ...
      component.element.addEventListener(
        ...
        (event: MouseEvent) => {
          const componentId =
            component.element.dataset.id || component.componentName || "nav";
    ...
    const getItemPath = (id: string): string[] => {
      ...
      const itemContainer = currentItem.element.closest(
        `.${prefix}-${NAV_CLASSES.ITEM_CONTAINER}`
      );
      if (!itemContainer) {
        return path;
      ...
      while (currentNestedContainer) {
        ...
        if (
          !parentItemContainer ||
          !parentItemContainer.classList.contains(
            `${prefix}-${NAV_CLASSES.ITEM_CONTAINER}`
          )
        ) {
          break;
        ...
        const parentItem = parentItemContainer.querySelector(
          `.${prefix}-${NAV_CLASSES.ITEM}[data-id]`
        );
        ...
        if (
          !grandparentElement ||
          !grandparentElement.classList.contains(
            `${prefix}-${NAV_CLASSES.NESTED_CONTAINER}`
          )
        ) {
          break;
