...
export const withAPI =
  ...
  (component: ComponentWithElements): ChipComponent => {
    ...
    let isSelected = component.element.classList.contains(
    ...
    // Track current variant
    const variantTypes: ChipVariant[] = [
      "filled",
      "outlined",
      "elevated",
      "assist",
    ...
    let currentVariant: ChipVariant | null = null;

    // Detect initial variant from class
    for (const v of variantTypes) {
      if (
        component.element.classList.contains(
          `${component.getClass("chip")}--${v}`,
        )
      ) {
        currentVariant = v;
        break;
    ...
    return {
      ...
      getValue() {
        ...
        const textElement = component.element.querySelector(
          `.${component.getClass("chip")}-text`,
        );
      ...
      enable() {
        ...
        component.element.classList.remove(
          `${component.getClass("chip")}--disabled`,
        );
      ...
      setText(content: string) {
        ...
        const contentContainer =
          component.element.querySelector(containerSelector) ||
          component.element;
        ...
        if (!textElement && content) {
          ...
          const leadingIcon = component.element.querySelector(
            `.${component.getClass("chip")}-leading-icon`,
          );
          if (leadingIcon) {
      ...
      setTrailingIcon(icon: string, onClick?: (chip: ChipComponent) => void) {
        ...
        let trailingIconElement =
          component.element.querySelector(trailingIconSelector);
      ...
      setVariant(variant: ChipVariant) {
        // Remove current variant class if exists
        if (currentVariant) {
          component.element.classList.remove(
            `${component.getClass("chip")}--${currentVariant}`,
          );
        }
        ...
        if (variant) {
          component.element.classList.add(
            `${component.getClass("chip")}--${variant}`,
          );
        }
