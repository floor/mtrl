...
export const createItemMeasurement = (defaultHeight: number = 48) => {
    ...
    if (useUniformHeight) {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i]) {
          ...
          const height = itemId && itemHeights.has(itemId) 
  ...
  return {
    ...
    measureItemHeight: (item: any, element: HTMLElement): number => {
      ...
      if (dynamicMeasurement || itemHeights.size === 0 || itemHeights.has(itemId)) {
      ...
      if (itemHeights.size === 1 && !dynamicMeasurement && element.offsetHeight > 0) {
    ...
    calculateTotalHeight: (items: any[]): number => {
      ...
      for (const item of items) {
        ...
        const itemId = getItemId(item);
        totalHeight += (itemId && itemHeights.has(itemId)) 
          ? itemHeights.get(itemId)! 
          : defaultHeight;
      }
    ...
    getItemOffset: (items: any[], itemId: string): number => {
      ...
      if (useUniformHeight) {
        ...
        for (const item of items) {
          ...
          const height = (currItemId && itemHeights.has(currItemId)) 
    ...
    measureMarkedElements: (container: HTMLElement, items: any[]): boolean => {
      ...
      const selector = itemHeights.size === 0 
        ? '.mtrl-list-item' 
