...
export function createInitialState(config: ListManagerConfig): ListManagerState {
  ...
  const initialItems = useStatic ? (config.staticItems || config.items || []) : [];
...
export function updateStateAfterLoad(
  ...
  newItems: any[],
  meta: { cursor?: string | null; hasNext?: boolean; total?: number; page?: number },
  dedupe: boolean = true
  ...
  if (state.paginationStrategy === 'page' && meta.page === undefined && !cursor) {
    page = state.page;
  }
  ...
  if (state.paginationStrategy === 'page' && state.page === 1) {
    ...
    updatedItems = [...state.items, ...itemsToAdd];
  }
