 ...
 * ```
 */
export const withPinchGesture = (config: PinchGestureConfig = {}) => 
  <C extends ElementComponent>(component: C): C & PinchGestureComponent => {
    ...
    if (onPinchStart) handlers.pinchstart.add(onPinchStart as (event: PinchEvent) => void);
    if (onPinchEnd) handlers.pinchend.add(onPinchEnd as (event: PinchEvent) => void);
