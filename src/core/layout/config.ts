...
import { addClass, hasClass } from "../dom/classes";
...
export function cleanupLayoutClasses(element: HTMLElement): void {
  ...
  const layoutClasses = classList.filter(
    (cls) => cls.startsWith("layout--") || cls.includes("-layout--")
  );
...
export function getLayoutType(element: HTMLElement): string {
  return hasClass(element, "layout--stack")
    ? "stack"
    : hasClass(element, "layout--row")
    ? "row"
    : hasClass(element, "layout--grid")
    ? "grid"
    : "";
}
...
export function applyLayoutClasses(
  ...
  if (layoutConfig.justify) {
    ...
    if (layoutType) {
      addClass(
        element,
        `layout--${layoutType}-justify-${layoutConfig.justify}`
      );
    }
  }
 ...
 */
export function applyLayoutItemClasses(
  element: HTMLElement,
  itemConfig: any
): void {
  ...
  if (itemConfig.span)
    addClass(element, `layout__item--span-${itemConfig.span}`);
  if (itemConfig.rowSpan)
    addClass(element, `layout__item--row-span-${itemConfig.rowSpan}`);
